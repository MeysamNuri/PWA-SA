...
import { DateEnum } from '../Hooks/topNMostRevenuable';
...
vi.mock('@/core/components/TotalCard/TotalCard', () => ({
    default: ({ rows }: any) => (
...
describe('TopNMostRevenuableProductsView', () => {
    ...
    it('renders without crashing', () => {
        mockUseTopNMostRevenuableProductsHook.mockReturnValue({
            ...
            loading: false,
            error: null,
            selectedChip: DateEnum.Last7Days,
    ...
    it('displays correct card rows with revenue filter', () => {
        ...
        expect(screen.getByTestId('row-0')).toHaveTextContent('مبلغ فروش: 50000000 تومان');
        expect(screen.getByTestId('row-1')).toHaveTextContent('مبلغ خرید: 40000000 تومان');
        expect(screen.getByTestId('row-2')).toHaveTextContent('مبلغ سود: 50000000 تومان');
    });
    ...
    it('displays total card with correct data when products are available', () => {
        mockUseTopNMostRevenuableProductsHook.mockReturnValue({
            topRevenuableProducts: [mockTopRevenuableProduct, mockTopRevenuableProduct2],
            loading: false,
            error: null,
        ...
        // Verify that the total card displays the expected texts.
        expect(screen.getByText(/مجموع مبلغ فروش/i)).toBeInTheDocument();
        expect(screen.getByText(/80000000/i)).toBeInTheDocument();
        expect(screen.getByText(/مجموع سود/i)).toBeInTheDocument();
        expect(screen.getByText(/15000000/i)).toBeInTheDocument();
    });

    it('displays total card with percentage data when filter is percentage', () => {
        mockUseTopNMostRevenuableProductsHook.mockReturnValue({
            topRevenuableProducts: [mockTopRevenuableProduct, mockTopRevenuableProduct2],
            loading: false,
            error: null,
        ...
        // Verify that the total card displays the expected percentage text.
        expect(screen.getByText(/میانگین درصد سود/i)).toBeInTheDocument();
        // expect(screen.getByText(/22.75%/i)).toBeInTheDocument();
    });
    ...
    it('handles date chip click correctly', () => {
        ...
        mockUseTopNMostRevenuableProductsHook.mockReturnValue({
            ...
            dateOptions: [
                ...
                { label: '۷ روز', value: DateEnum.Last7Days },
                { label: '۳۰ روز', value: DateEnum.Last30Days }
            ]
        } as any);
        ...
        // expect(mockHandleChipClick).toHaveBeenCalledWith(DateEnum.Yesterday);
    });
    ...
    it('displays correct date options', () => {
        ...
        expect(screen.getByText('۷ روز گذشته')).toBeInTheDocument();
