...
import useTopNMostSoldProductsHook, { DateEnum } from './topNMostSold';
...
describe('useTopNMostSoldProductsHook', () => {
  ...
  it('should initialize with default values and fetch data', () => {
    ...
    // Check initial state
    expect(result.current.selectedChip).toBe(DateEnum.Last7Days.toString());
    ...
    expect(useTopNMostSoldProducts).toHaveBeenCalledWith(DateEnum.Last7Days.toString());
  ...
  it('should initialize with state from useLocation', () => {
    ...
    expect(result.current.selectedChip).toBe(DateEnum.Yesterday.toString());
    ...
    expect(useTopNMostSoldProducts).toHaveBeenCalledWith(DateEnum.Yesterday.toString());
  ...
  it('should update selectedChip when handleChipClick is called', () => {
    ...
    act(() => {
      result.current.handleChipClick(DateEnum.Last30Days);
    });

    // Check if the selected chip has updated
    expect(result.current.selectedChip).toBe(DateEnum.Last30Days.toString());
    // Verify that useTopNMostSoldProducts is called again with the new chip value
    expect(useTopNMostSoldProducts).toHaveBeenCalledWith(DateEnum.Last30Days.toString());
  });
