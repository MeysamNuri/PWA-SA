...
const RenderProductCard: React.FC<ProductCardProps> = ({ product, filterByRevenue }) => {
    const baseRows = [
        {
            title: "مبلغ فروش",
            value: `${NumberConverter.formatCurrency(Math.floor(product.saleAmount / 10))} تومان`,
            rowSize: 12
        },
        {
            title: "مبلغ خرید",
            value: `${NumberConverter.formatCurrency(Math.floor(product.purchaseAmount / 10))} تومان`,
            rowSize: 12
        }
    ];
    ...
    const conditionalRow = filterByRevenue
        ? {
            title: "مبلغ سود",
            value: `${NumberConverter.formatCurrency(Math.floor(product.salesRevenuAmount / 10))} تومان`,
            rowSize: 12
        }
        : {
            title: "درصد سود",
            value: `${NumberConverter.latinToArabic(product.revenuPercentage?.toString() || '0')}%`,
            rowSize: 12
        };
