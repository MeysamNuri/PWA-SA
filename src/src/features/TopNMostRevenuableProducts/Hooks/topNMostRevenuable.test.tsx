...
import { useTopNMostRevenuableProductsHook } from './topNMostRevenuable'; // Fixed import path
...
describe('useTopNMostRevenuableProductsHook', () => {
  ...
  it('باید با مقادیر پیش‌فرض مقداردهی اولیه شود و داده‌ها را با موفقیت دریافت کند', async () => {
    ...
    expect(result.current.selectedChip).toBe(DateFilterType.Last7Days);
    ...
    expect(useTopNMostRevenuableProducts).toHaveBeenCalledWith(DateFilterType.Last7Days);
  });

  it('باید با مقادیر از location state مقداردهی اولیه شود و داده‌ها را با موفقیت دریافت کند', async () => {
    (useLocation as any).mockReturnValue({
      state: { date: DateFilterType.Yesterday, filterByRevenue: false },
    });
    ...
    expect(result.current.selectedChip).toBe(DateFilterType.Yesterday);
    ...
    expect(useTopNMostRevenuableProducts).toHaveBeenCalledWith(DateFilterType.Yesterday);
  });
  ...
  it('باید selectedChip را زمانی که handleChipClick فراخوانی می‌شود به‌روزرسانی کند و داده‌ها را مجدداً دریافت کند', async () => {
    ...
    act(() => {
      result.current.handleChipClick(DateFilterType.Yesterday);
    });
