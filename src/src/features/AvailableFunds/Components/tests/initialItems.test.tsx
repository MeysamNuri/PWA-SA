...
describe('InitialItemsSection', () => {
    // Test case 1: Renders bank items when valueType is 'bank' with toman currency
    it('should render bank items when valueType is "bank" with toman currency', () => {
        render(
            <InitialItemsSection
                valueType="bank"
    ...
    // Test case 2: Renders fund items when valueType is 'fund' with toman currency
    it('should render fund items when valueType is "fund" with toman currency', () => {
        render(
            <InitialItemsSection
                valueType="fund"
                initialBankItems={[]}
    ...
    // Test case 3: Renders bank items when valueType is 'bank' with dollar currency
    it('should render bank items when valueType is "bank" with dollar currency', () => {
        render(
            <InitialItemsSection
                valueType="bank"
                initialBankItems={mockBankItems}
        ...
        // Check for rendered content from mockBankItems with dollar values
        expect(screen.getByText('۱٬۰۰۰٬۰۰۰')).toBeInTheDocument();
        expect(screen.getByText('حساب بانکی اول')).toBeInTheDocument();
     
    });

    // Test case 4: Renders fund items when valueType is 'fund' with dollar currency
    it('should render fund items when valueType is "fund" with dollar currency', () => {
        render(
            <InitialItemsSection
                valueType="fund"
                initialBankItems={[]}
        ...
        // Check for rendered content from mockFundItems with dollar values
        expect(screen.getByText('۵٬۰۰۰٬۰۰۰')).toBeInTheDocument();
    
        expect(screen.getByText('صندوق سرمایه‌گذاری')).toBeInTheDocument();
    });

    // Test case 5: Renders an empty state if no data is provided
